...
module.exports = {
  data: new SlashCommandBuilder()
    ...
    .setDescription('Envoyer une annonce pour le pôle illégal')
    ...
    .addStringOption(option =>
      option
        .setName('couleur')
        .setDescription('Couleur HEX, exemple : #ED4245')
        .setRequired(false)
    ),
  ...
  async execute(interaction) {
    if (!hasPermission(interaction.member, 'annonceillegal')) {
      return interaction.reply({
        content: '❌ Tu n’as pas la permission d’utiliser `/annonceillegal`.',
        ephemeral: true
      });
    }
    ...
    return sendAnnouncement(interaction, {
      type: 'illegal',
      title: interaction.options.getString('titre') || '🔫 Annonce Illégal',
      ...
      logTitle: '🔫 Annonce illégal envoyée'
    });
  }
};